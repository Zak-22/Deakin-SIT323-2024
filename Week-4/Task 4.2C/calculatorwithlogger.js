// Task 4.2C - Extended Calculator functions
...
const exponentiation = (n1, n2) => {
  return Math.pow(n1, n2);  // Math. funtion used for calculating the power of a number.
}
const squareRoot = (n) => {
  if (n < 0) {
    throw new Error("Square root of a negative number is undefined");
  }
  return Math.sqrt(n);  // Math. function used to calculate square root of a number. Only single value is needed.
}
const modulo = (n1, n2) => {
  if (n2 === 0) {
    throw new Error("Modulo by zero");
  }
  return n1 % n2; // % operator to calculate the remainder if 2 numbers are divided.
}
...
app.get("/exponentiation", (req, res) => {
  try {
    ...
    const expResult = exponentiation(n1, n2);
    res.status(200).json({ statusCode: 200, data: expResult });
  } catch (error) {
    logger.error(error.toString());
    res.status(400).json({ statusCode: 400, msg: error.toString() });
  }
...
app.get("/square-root", (req, res) => {
  try {
    ...
    const sqrtResult = squareRoot(n);
    res.status(200).json({ statusCode: 200, data: sqrtResult });
  } catch (error) {
    logger.error(error.toString());
    res.status(400).json({ statusCode: 400, msg: error.toString() });
  }
...
app.get("/modulo", (req, res) => {
  try {
    ...
    const modResult = modulo(n1, n2);
    res.status(200).json({ statusCode: 200, data: modResult });
  } catch (error) {
    logger.error(error.toString());
    res.status(400).json({ statusCode: 400, msg: error.toString() });
  }
